/* global browser, getMatchingRules, applyModifyReplacementsToContext, runNetworkRuleScript,
  NETWORK_RULES_LOG_KEY, trimLogEntries, getSortedEnabledRules, rulesForPageHook,
  applyStringReplacements, objectToWebRequestHeaders, webRequestHeadersToObject,
  isTextLikeContentType, sanitizeLogEntry, createSharedStateView */
...
async function logWebRequestRule(entry) {
  ...
  const entries = trimLogEntries([
    ...(stored[NETWORK_RULES_LOG_KEY] || []),
    sanitized,
  ]);
...
function syncNetworkWebRequest(state, hooksEnabled = true, sharedState = {}) {
  ...
  webRequestRules = getSortedEnabledRules(state?.rules || []);
