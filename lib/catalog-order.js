 ...
 */
(function () {
  const CATALOG_ORDER_KEY = "catalogOrder";

  /**
   * Stable key for shortcuts and ordering.
   * Custom links use id; bundled use section/folder path/name.
   */
  function linkStableKey(sectionName, pathParts, node) {
    if (node?.id) {
      return `id:${node.id}`;
    }
  ...
  function folderStableKey(sectionName, pathParts, folderName) {
    const parts = [sectionName, ...(pathParts || []), folderName || ""]
      .map((p) => String(p).replace(/\//g, "\\/"));
    return `folder:${parts.join("/")}`;
  }

  function walkWithKeys(nodes, sectionName, pathParts, visitor) {
  ...
  function collectKeysFromCatalog(catalog) {
    const keys = [];
    const sectionOrder = [];
   ...
   */
  function applyOrder(catalog, orderState) {
   ...
   */
  function moveKeysInOrder(linkKeys, orderedSiblingKeys) {
  ...
  async function saveCatalogOrder(orderState) {
  ...
  function appendMissingKeys(orderState, catalog) {
  ...
  globalThis.SnLinksCatalogOrder = {
    CATALOG_ORDER_KEY,
    linkStableKey,
    folderStableKey,
    walkWithKeys,
    collectKeysFromCatalog,
    applyOrder,
    moveKeysInOrder,
    loadCatalogOrder,
    saveCatalogOrder,
  ...
  // Mirror key for background script list (var for cross-file scope).
  var CATALOG_ORDER_KEY_BG = CATALOG_ORDER_KEY;
})();
