...
function normalizeStringSource(source) {
  ...
  throw new Error(`Unknown extract stringSource: ${source}`);
}

function normalizeExtractEntry(paramName, spec) {
  if (!spec || typeof spec !== "object") {
    throw new Error(`Invalid extract spec for ${paramName}.`);
  }
  if (spec.selector) {
    const stringSource = normalizeStringSource(spec.stringSource);
    if (stringSource === "attribute" && !spec.attribute) {
      throw new Error(
        `Extract for ${paramName} requires attribute when stringSource is attribute.`
      );
    }
    return {
      paramName,
  ...
  if (spec.url) {
    return { paramName, kind: "url", pattern: spec.url };
  }
  throw new Error(
    `Extract for ${paramName} requires url (regex) or selector (DOM).`
  );
}
...
function normalizeExtractEntries(extract) {
  ...
  if (typeof extract !== "object" || Array.isArray(extract)) {
    throw new Error("extract must be an object mapping parameter names to extract specs.");
  }
  return Object.entries(extract).map(([paramName, spec]) =>
    normalizeExtractEntry(paramName, spec)
  );
}
...
function getParameterDefNames(node) {
  ...
  for (const name of getExtractParamNames(node)) {
    if (!names.includes(name)) {
      names.push(name);
    }
  }
...
function nodeHasOnLoad(node) {
  return node.type === "scriptlet" && !node.nav;
}
