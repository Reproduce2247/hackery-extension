...
export const LINKS_SCHEMA_VERSION = 3;
...
export function normalizeExtractEntry(paramName, spec) {
...
export function normalizeExtractEntries(extract) {
  if (!extract) {
    return [];
  }
...
export function getExtractParamNames(node) {
  const navParams = getNavParamsObject(node);
  if (!navParams) {
    return [];
  }
...
export function applyParameters(template, values) {
...
export function resolveHostPattern(node, inherited) {
...
export function normalizeLeafNode(node) {
  ...
  // Prefer legacy displayName when present so old overlays keep their UI label.
  if (node.displayName) {
    out.name = node.displayName;
  } else if (node.name) {
    out.name = node.name;
  }
...
export function catalogNeedsNormalization(raw) {
