...
const DNR_RESOURCE_TYPES = ["main_frame", "sub_frame"];
...
/**
 * No domain filter: every document in the tab is covered, including origins
 * that had not loaded when the checkbox was ticked (ad rotation, redirect
 * chains, lazily inserted iframes). `tabIds` is session-rule-only, which is
 * what updateSessionRules installs. Listing the tab also excludes tabId -1.
 */
function nonceTabToDnr(tabId, index) {
  return dnrRemoveCspRule(DNR_NONCE_ID_BASE + index, { tabIds: [tabId] });
}
...
export async function syncCspDnrRules(networkState, networkArmed) {
  ...
  tabs.forEach((tabId, index) => {
    next.push(nonceTabToDnr(tabId, index));
  });
