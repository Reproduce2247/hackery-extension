...
export function migrateCustomScripts(scripts) {
...
async function runSchemaMigrationIfNeeded(overlay) {
  const { LINKS_SCHEMA_VERSION_KEY, LINKS_OVERLAY_KEY, PARAM_VALUES_KEY, INJECT_ON_LOAD_KEY } =
    StorageKeys;

  const stored = await browser.storage.local.get([LINKS_SCHEMA_VERSION_KEY]);
  const currentVersion = stored[LINKS_SCHEMA_VERSION_KEY] || 0;
  if (currentVersion >= LINKS_SCHEMA_VERSION) {
    return overlay;
  }

  let nextOverlay = overlay || {};
  if (catalogNeedsNormalization(nextOverlay)) {
    nextOverlay = normalizeCatalog(nextOverlay);
  }

  await browser.storage.local.set({
    [LINKS_OVERLAY_KEY]: nextOverlay,
    [LINKS_SCHEMA_VERSION_KEY]: LINKS_SCHEMA_VERSION,
    [PARAM_VALUES_KEY]: {},
    [INJECT_ON_LOAD_KEY]: {},
  });
...
export async function ensureLinksOverlayInStorage() {
  const { LINKS_OVERLAY_KEY, CUSTOM_SCRIPTS_KEY } = StorageKeys;
  ...
  overlay = await runSchemaMigrationIfNeeded(overlay);

  let idsBackfilled = false;
  const withIds = {};
  for (const [sectionName, section] of Object.entries(overlay)) {
    if (!section || typeof section !== "object") {
      withIds[sectionName] = section;
      continue;
    }
    const { nodes, changed } = ensureLinkIdsInTree(section.children);
    if (changed) {
      idsBackfilled = true;
      withIds[sectionName] = { ...section, children: nodes };
    } else {
      withIds[sectionName] = section;
    }
  }
...
export { LEGACY_CUSTOM_MIGRATION_SECTION };
