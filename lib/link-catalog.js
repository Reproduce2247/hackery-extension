(function () {
  ...
  function mergeLinksCatalog(bundled, overlay) {
    ...
    const normalizedBundled = LM.normalizeCatalog(bundled);
    if (!overlay || typeof overlay !== "object") {
      return normalizedBundled;
    }

    const normalizedOverlay = LM.normalizeCatalog(overlay);
  ...
  function validateImportNode(node, indexLabel) {
    ...
    const LM = globalThis.SnLinksLinkModel;
    const normalized = LM.normalizeLeafNode(node);
    const behavior = globalThis.SnLinksBehaviors?.matchBehavior?.(normalized);
    if (!behavior) {
  ...
  function normalizeImportedNodes(nodes) {
    ...
    return nodes.map((node) => {
      if (node.children) {
        return {
          ...LM.normalizeTreeNode(node),
          children: normalizeImportedNodes(node.children),
        };
      }
      return ensureLinkId(LM.normalizeLeafNode(node));
    });
  }
  ...
  async function runSchemaMigrationIfNeeded(overlay) {
    ...
    if (currentVersion >= LM.LINKS_SCHEMA_VERSION) {
      return overlay;
    }

    let nextOverlay = overlay || {};
    if (LM.catalogNeedsNormalization(nextOverlay)) {
      nextOverlay = LM.normalizeCatalog(nextOverlay);
    }

    await browser.storage.local.set({
      [LINKS_OVERLAY_KEY]: nextOverlay,
      [LINKS_SCHEMA_VERSION_KEY]: LM.LINKS_SCHEMA_VERSION,
      [PARAM_VALUES_KEY]: {},
      [INJECT_ON_LOAD_KEY]: {},
    });
