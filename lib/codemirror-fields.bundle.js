...
var StreamLanguage = class _StreamLanguage extends Language {
  constructor(parser3) {
    let data = defineLanguageFacet(parser3.languageData);
    let p = fullParser(parser3), self;
    let impl = new class extends Parser {
      createParse(input, fragments, ranges) {
        return new Parse(self, input, fragments, ranges);
      }
    }();
    super(data, impl, [], parser3.name);
    this.topNode = docID(data, this);
    self = this;
    this.streamParser = p;
    this.stateAfter = new NodeProp({ perNode: true });
    this.tokenTable = parser3.tokenTable ? new TokenTable(p.tokenTable) : defaultTokenTable;
  }
  /**
...
var TokenGroup = class {
  ...
  token(input, stack) {
    ...
    readToken2(this.data, input, stack, this.id, parser3.data, parser3.tokenPrecTable);
  }
};
...
// lib/codemirror-fields.js
var editors = /* @__PURE__ */ new WeakMap();
var regexHighlightMode = simpleMode({
  start: [
    { regex: /\\./, token: "escape" },
    { regex: /\(\?:|\(\?[=!<:]/, token: "meta" },
    { regex: /[[\](){}|]/, token: "bracket" },
...
var networkScriptScopeSource = scopeCompletionSource(NETWORK_SCRIPT_SCOPE);
...
function languageExtension(language2) {
  ...
  if (language2 === "plain" || language2 === "wildcard") {
    return [];
  }
  return javascript();
}
...
function lintExtension(language2, compact, enabled) {
  if (!enabled) {
    return [];
  }
  let source = null;
...
function createExtensions({
  ...
  const extensions = [
    ...
    closeBrackets(),
    editorTheme(compact),
    languageCompartment.of(languageExtension(language2)),
    lintCompartment.of(lintExtension(language2, compact, lint)),
  ...
  if (placeholder2) {
    extensions.push(placeholderCompartment.of(placeholder(placeholder2)));
  } else {
    extensions.push(placeholderCompartment.of([]));
  }
  return extensions;
}
function attachCodeMirror(element, options = {}) {
  if (!element) {
    return null;
  }
  ...
  const lintEnabled = options.lint ?? (language2 === "javascript" || language2 === "json" || language2 === "regex");
  const completions = options.completions ?? (language2 === "javascript" ? "javascript" : false);
  ...
  const api = {
    ...
    setLanguage(nextLanguage) {
      const nextLint = options.lint ?? (nextLanguage === "javascript" || nextLanguage === "json" || nextLanguage === "regex");
      view.dispatch({
        effects: [
...
function setFieldLanguage(element, language2) {
  if (!element) {
    return;
  }
  editors.get(element)?.setLanguage(language2);
}
function setFieldPlaceholder(element, placeholder2) {
  if (!element) {
    return;
  }
  element.placeholder = placeholder2 || "";
  editors.get(element)?.setPlaceholder(placeholder2 || "");
}
...
function attachCodeMirrorAll(entries) {
  const attached = {};
  for (const [key, config2] of Object.entries(entries)) {
    const { element, ...options } = config2;
    if (!element) {
      continue;
    }
