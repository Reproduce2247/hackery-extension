...
function createExtensions({ language: language2, compact, minHeight, placeholder: placeholder2 }) {
  const extensions = [
...
function attachCodeMirror(element, options = {}) {
  ...
  const { language: language2 = "javascript", compact = false, minHeight, placeholder: placeholder2 } = options;
  ...
  const view = new EditorView({
    state: EditorState.create({
      doc: element.value,
      extensions: createExtensions({ language: language2, compact, minHeight, placeholder: placeholder2 })
    }),
    parent: wrapper
  });
  const api = {
