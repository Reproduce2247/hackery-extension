 ...
 * @param {string} language Language id (`javascript`, `json`, `regex`, `plain`, `wildcard`).
 * @returns {import("@codemirror/state").Extension}
 */
 ...
 * Skips blank docs and URL/path-only paste values.
 * @param {EditorView} view Editor view to lint.
 * @returns {import("@codemirror/lint").Diagnostic[]}
 */
function javascriptSyntaxLinter(view) {
  ...
  // Sidebar quick-add accepts a URL/path or JS; don't flag bare URLs as syntax errors.
 ...
 * @param {number} [options.minHeight] Minimum scroller height in px.
 * @param {string} [options.placeholder] Placeholder text.
 * @param {boolean} [options.lint] Override lint enablement (default: on for json/regex/javascript).
...
export function attachCodeMirror(element, options = {}) {
  ...
  const lintEnabled =
    options.lint ??
    (language === "javascript" || language === "json" || language === "regex");
  const completions =
